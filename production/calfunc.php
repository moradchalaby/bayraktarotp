...
<main class="calendar-contain">
    <section class="title-bar">
        <a href="javascript:void(0);" class="title-bar__prev"
            onclick="getCalendar('calendar_div','<?php echo date("Y", strtotime($date . ' - 1 Month')); ?>','<?php echo date("m", strtotime($date . ' - 1 Month')); ?>');"></a>
    ...
    <aside class="calendar__sidebar">
        ...
        <a href="javascript:void(0);" class="add-event-btn btn">+add event</a>
        ...
        <?php
            ...
            for ($cb = 1; $cb <= $boxDisplay; $cb++) {
                if (($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)) {
                    ...
                    global $dbh;
                    $result = $dbh->query("SELECT title FROM events WHERE date = '" . $currentDate . "' AND status = 1");
...
function getEvents($date = '')
{
    ...
    global $dbh;
    $result = $dbh->query("SELECT title FROM events WHERE date = '" . $date . "' AND status = 1");
...
function addEvent($postData)
{
    global $dbh;
    $status =
        "Error description: " . $dbh->error;;
    if (!empty($postData['event_title']) && !empty($postData['event_date'])) {
        global $dbh;
        $event_title = $dbh->real_escape_string($postData['event_title']);
        $insert = $dbh->query("INSERT INTO events (title, date) VALUES ('" . $event_title . "', '" . $postData['event_date'] . "')");
        if ($insert) {
            $status = 1;
        } else {
            $status = "Error description: " . $dbh->error;
        }
    } else {
        $status = 'boşş';
    }
